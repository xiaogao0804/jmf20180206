       ...
       <div class="con clearFix">
			<div class="con-left">
               <ul>
                   ...
                   <li id="t">
                   		<img src="<%=path %>/jiumenfang-web/WeChat/storeKeeper/src/jsp/tubeBack/src/image/tuihuanSelect.png" alt="">
                   </li>
                 <%--   <li>
                       <a href="cha.do">
           ...
           <div class="con-right clearFix">
               <div class="returns-page page clearFix">
                   <div class="order-inquire clearFix">
                      <div class="goods-date">
				       <form action="chaTui.do" method="post">
                   ...
                   <div class="order-list">
                       <table class="xxx">
                          ...
                          <c:forEach items="${list }" var="l">
                          	<tr>
                          		<input type="hidden" value="${l.upload1_id }" class="upload1_id">
                          		<td style="text-indent:30px;">${l.uploadDate1 }</td>
                          		<td>${l.kuaidi }</td>
                          		<td>${l.yundan }</td>
                          		<td>${l.chanpin }</td>
                          		<td>${l.num }</td>
                          		<td>${l.user_name }</td>
                          		<td class="a1${l.upload1_id }">
                           			<c:forEach items="${list1 }" var="ll">
                           				<input type="image" class="m${l.upload1_id }" value="${ll.path1_id }" src="${ll.uploadPath1 }" width="100px;" height="100px;" onclick="closeShadeLayer();return false;">
...
function fenye(page){
	...
	$(function(){
		...
		$(".xxx").find('tr:even').css('background','#DFE9EB');
		$(".xxx").find('tr:odd').css('background','#fff');
...
$(".out").click(function(){
	...
	window.location.href="outExcel3.do";
		});
	});
	$.ajax({
		url:"tui.do",
		...
		success:function(msg){
			for(var i in msg){
				for(var s1 in msg[i]){
					var s2=".m"+msg[i][s1].upload1_id;
						if(s2=1){
							$(".a1"+msg[i][s1].upload1_id).append("<a href="+msg[i][s1].uploadPath1+"><input type='image' class='ssssss' style='height: 100px;width: 100px;' value="+msg[i][s1].uploadPath1+" src="+msg[i][s1].uploadPath1+" onclick='tupian('+msg[i][s1].uploadPath+')'>'</a>");
						}	
				}
			}
		}
	...
	$(".sc").each(function(){
		$(this).click(function(){
			var upload1_id=$(this).parent('td').siblings('.upload1_id').val();
			 $.ajax({
				url:"delectUpload1.do",
				type:"post",
				data:{"upload1_id":upload1_id},
				success:function(msg){
					alert("删除成功");
					location.reload();
				}
