...
import com.jiumenfang.user.bean.AccessToken;
...
//		return jsonObject;
	...
	public static JsonObject getOpenid(String appid , String appsecret,String code) throws Exception, IOException {
		JsonParser jsonparer = new JsonParser();// 初始化解析json格式的对象
...
//			if(s.getDateTime()>overduetime){
			...
			if(currentTimeMillis>overduetime){
				//超时从新获取accessToken
				token = util.getAccessToken2(Constants.APPID, Constants.SECRET);
				putil.writeProperties("OVERDUE_ACCESS_TOKEN_TIME", String.valueOf(currentTimeMillis+Constants.EFFECTIVE_TIME));
	...
	public static JsonObject getOpenidInfo(String openid, String appid, String appsecret) throws Exception {
		JsonParser jsonparer = new JsonParser();// 初始化解析json格式的对象
		System.out.println("getAccessToken(appid, appsecret):>>>>"+getAccessToken(appid, appsecret));
		System.out.println("openid:>>>>"+openid);
		String requestUrl = "https://api.weixin.qq.com/cgi-bin/user/info?access_token="+getAccessToken(appid, appsecret).getAccessToken()+"&openid="+openid+"&lang=zh_CN";
		HttpClient client = new DefaultHttpClient();
		HttpGet get = new HttpGet(requestUrl);
		HttpResponse res = client.execute(get);
		String responseContent = null; // 响应内容
