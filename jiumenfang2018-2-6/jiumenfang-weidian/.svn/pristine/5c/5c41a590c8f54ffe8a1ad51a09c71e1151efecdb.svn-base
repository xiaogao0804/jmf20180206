...
describe('gulp-livereload', function() {
  ...
  it('reload', function() {
    ...
    glr.reload('not-shou.html');
    assert(spy.secondCall.calledWith(files('not-shou.html')));
  });
  ...
  it('option: reloadPage', function() {
    var spy = sinon.spy();
    srv.returns({ changed: spy, listen: function() {} });
    glr.listen({ reloadPage: 'not-shou.html' });
    glr.reload();
    assert(spy.calledWith(files('not-shou.html')));
  });
});
